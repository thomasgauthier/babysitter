...
- do not activate unrelated built-in skills or broaden into unrelated workflows
- if the named skill is local to this repo, read its `SKILL.md` directly and follow it
...
- for `Reject`, prefer the one-click `reject <id>` path when you want the fixed host reason `User rejected action`
...
- do not approve shell-based file writes (for example `bash`, `echo >`, `cat >`, or `printf >`) when the model should be using a dedicated `write` or `edit` tool; reject it or nudge it back to the correct tool
- if the model is looping on an identical or near-identical write, do not keep approving it; give one exact nudge or use `reject` and push it to the next bounded step
...
Requirements:
- use `qwen3.5-9b-ud` as the small model
- you are the frontier babysitter
- launch the run with `babysitter new --model qwen3.5-9b-ud`
...
- be patient: if `babysitter status` says `running`, the agent is still running, so keep polling until you have explicit stop evidence
